...
class Room extends EventEmitter
{
	...
	handleProtooConnection({ peerId, consume, protooWebSocketTransport })
	{
		...
		peer.data.audioTransport = undefined;
		peer.data.videoTransport = undefined;
		peer.data.audioConsumer = undefined;
		peer.data.videoConsumer = undefined;
		peer.data.useAudio = false;
		peer.data.useVideo = false;
		...
		peer.data.directProducers = new Map();
		...
		/* peer.on('handleDirectProducer', (producer) =>
		{
			if (producer.kind === 'video' || producer.kind === 'audio')
			{
				peer.data.directProducers.set(producer.kind, producer);
			}
			if (peer.data.directProducers.size === 2)
			{
				this.prepareRecording(peer);
			}
		});*/
	}
	...
	async _handleProtooRequest(peer, request, accept, reject)
	{
		switch (request.method)
		{
			...
			case 'startRecording':
			{
				...
				const audioProducer = peer.data.producers.get(audioProducerId);
				const videoProducer = peer.data.producers.get(videoProducerId);

				if (audioProducerId && audioProducer)
				{
					peer.data.useAudio = true;
					const audioRtpTransport = await this._mediasoupRouter.createPlainTransport(
						{
							comedia : false,
							rtcpMux : false,
					...
					peer.data.audioTransport = audioRtpTransport;
					
					await audioRtpTransport.connect({
						ip       : config.mediasoup.recordingOptions.ip,
						port     : config.mediasoup.recordingOptions.audioPort,
						rtcpPort : config.mediasoup.recordingOptions.audioPortRtcp
					});

					const audioRtpConsumer = await audioRtpTransport.consume({
						producerId      : audioProducerId,
						rtpCapabilities : rtpCapabilities,
						paused          : true
					});

					peer.data.audioConsumer = audioRtpConsumer;
				}
				if (videoProducerId && videoProducer)
				{
					peer.data.useVideo = true;
					const videoRtpTransport = await this._mediasoupRouter.createPlainTransport(
						{
							comedia : false,
							rtcpMux : false,
					...
					peer.data.videoTransport = videoRtpTransport;

					await videoRtpTransport.connect({
						ip       : config.mediasoup.recordingOptions.ip,
						port     : config.mediasoup.recordingOptions.videoPort,
						rtcpPort : config.mediasoup.recordingOptions.videoPortRtcp
					});

					const videoRtpConsumer = await videoRtpTransport.consume({
						producerId      : videoProducerId,
						rtpCapabilities : rtpCapabilities,
						paused          : true
					});

					peer.data.videoConsumer = videoRtpConsumer;
				}

				// Start recording.
				//await this.startRecording(peer);

				if (peer.data.useAudio)
				{
					const consumer = peer.data.audioConsumer;
				...
				if (peer.data.useVideo)
				{
					const consumer = peer.data.videoConsumer;
			...
			case 'createWebRtcTransport':
			{
				...
				logger.warn('!!!!!! createWebRtcTransport request.data', request.data);
				...
				logger.warn('!!!!!! createWebRtcTransport webRtcTransportOptions', webRtcTransportOptions);
				...
				logger.warn('!!!!!! createWebRtcTransport transport', transport);
			...
			case 'connectWebRtcTransport':
			{
				...
				logger.warn('!!!!!! connectWebRtcTransport transportId', request.data);
				const transport = peer.data.transports.get(transportId);
				logger.warn('!!!!!! connectWebRtcTransport transport', transport);
				if (!transport)
			...
			case 'produce':
			{
				...
				peer.emit('handleDirectProducer', producer);
	...
	// Return a Promise that can be awaited
		...
		const outputDir = path.join(recordingDir, dateFolderName);
		...
		let cmdInputPath = path.join(recordingDir, 'input-vp8.sdp');
		let cmdOutputPath = path.join(outputDir, `output-${peer.id}.webm`);

		const cmdInputPath2 = path.join(recordingDir, 'input-h264.sdp');
		const cmdOutputPath2 = path.join(outputDir, `output-${peer.id}.mp4`);

		const useVideo = peer.data.useVideo;
		const useAudio = peer.data.useAudio;
		const useH264 = false;
		const autoStop = false;
		...
		if (!ffmpegOk) 
		{
			logger.error('FFmpeg >= 4.0.0 not found in $PATH; please install it');
			process.exit(1);
		}
		...
		if (useVideo) 
		{
			...
			if (useH264) 
			{
				cmdInputPath = cmdInputPath2;
				cmdOutputPath = cmdOutputPath2;

				// "-strict experimental" is required to allow storing
				// OPUS audio into MP4 container
				cmdFormat = '-f mp4 -strict experimental -shortest';
			}
		}

		// cmdCodec = '-map 0:1 -c:a copy -c:v copy -b:v 1M -s 1280x720';
		...
		logger.info(`Run command: ${cmdProgram} ${cmdArgStr}`);

		const recProcess = Process.spawn(cmdProgram, cmdArgStr.split(/\s+/));
		
		if (autoStop)
		{
			const recordingDuration = 5000; // in milliseconds (5 seconds)
			const stopRecordingTimer = setTimeout(() =>
			{
				logger.error('Recording stopped by timer');
	...
	stopRtp(peer)
	{
		logger.info('Stopping peer RTP');
		if (typeof peer.data.audioConsumer !== 'undefined' && typeof peer.data.audioTransport !== 'undefined')
		{
			peer.data.audioConsumer.close();
			peer.data.audioTransport.close();
		}
		if (typeof peer.data.videoConsumer !== 'undefined' && typeof peer.data.videoTransport !== 'undefined')
		{
			peer.data.videoConsumer.close();
			peer.data.videoTransport.close();
		}
	}

	async prepareRecording(peer)
	{
		/* const tempFilePath = path.join(__dirname, '..', 'recording', `${Date.now()}.mp4`);

		const videoConsumer = await this.createProducerDirectTransport(peer.data.directProducers.get('video'), peer.data.rtpCapabilities);
		const audioConsumer = await this.createProducerDirectTransport(peer.data.directProducers.get('audio'), peer.data.rtpCapabilities);
		logger.info('videoConsumer', videoConsumer);
		logger.info('audioConsumer', audioConsumer);
		...
		const directConsumer = await directTransport.consume({
			producerId      : peer.data.directProducers.get('video').id,
		...
		{
			directProducer.send(rtpPacket);

			fileWriteStream.write(rtpPacket);
			// Pipe RTP packets to ffmpeg
		});

		directTransport.on('rtcp', (buffer) => 
		{
			logger.info('rtcp', buffer);
			directTransport.sendRtcp(buffer);
		});*/
	}

	 /* async createProducerDirectTransport(producer, rtpCapabilities)
	{
		if (!producer) return;

		const directTransport = await this._mediasoupRouter.createPlainTransport({
			comedia  : false,
			rtcpMux  : false,
		...
		await directTransport.connect({
			ip       : config.mediasoup.recordingOptions.ip,
			port   	 : producer.kind === 'video' ? config.mediasoup.recordingOptions.videoPort : config.mediasoup.recordingOptions.audioPort,
			rtcpPort : producer.kind === 'video' ? config.mediasoup.recordingOptions.videoPortRtcp : config.mediasoup.recordingOptions.audioPortRtcp
		});
